...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .total  { font-size: 11px; font-weight: bold; color: #333; margin-bottom: 10px; }
    </style>
</head>
<body>

    <div class="header">
        <h2><i></i> Beneficiary Registration Report</h2>
    ...
    <div class="meta">
        ...
        <span class="total">Total: {{ $beneficiaries->count() }} beneficiaries</span>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $b->email ?? '—' }}</td>
                <td><span class="badge">{{ $b->sex ?? '—' }}</span></td>
                <td>{{ $b->age ?? '—' }}</td>
                <td>{{ $b->contact_number ?? '—' }}</td>
                <td>{{ $b->guardian_name ?? '—' }}</td>
                <td>
                    <div>{{ \Carbon\Carbon::parse($b->date_registered)->format('M d, Y') }}</div>
